...
export default function ChatOnlineUsers({ 
  ...
  useEffect(() => {
    ...
    window.addEventListener('socket_user_online', handleUserOnline as EventListener);
    window.addEventListener('socket_user_offline', handleUserOffline as EventListener);

    // Request current online users
    socketService.getOnlineUsers();

    return () => {
      window.removeEventListener('socket_user_online', handleUserOnline as EventListener);
      window.removeEventListener('socket_user_offline', handleUserOffline as EventListener);
    };
  }, []);
        ...
        <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
          <div className="p-4 border-b border-gray-200">
            ...
            <p className="text-xs text-gray-500 mt-1">
              {t('chat.onlineCount', { count: onlineParticipants.length, total: participants.length })}
            </p>
          </div>
