...
export function SubmissionReviewDialog({ assignmentId, isOpen, onClose }: SubmissionReviewDialogProps) {
    ...
    const downloadFile = (path: string) => {
        ...
        const fileUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/${path.replace(/\\/g, '/')}`;
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>Review student submissions and award grades.</DialogDescription>
                </DialogHeader>
                        ...
                        <TableBody>
                            {submissions.map((sub) => (
                                <TableRow key={sub.student._id}>
                                    ...
                                    <TableCell>
                                        {sub.file && (
                                            <Button variant="ghost" size="sm" onClick={() => downloadFile(sub.file)}>
                                    ...
                                    <TableCell>
                                        <Input
                                            ...
                                            onChange={(e) => handleGradeChange(sub.student._id, e.target.value)}
                                        />
                                    </TableCell>
                                    <TableCell>
                                        <Button size="sm" variant="ghost" onClick={() => saveGrade(sub.student._id)}>
