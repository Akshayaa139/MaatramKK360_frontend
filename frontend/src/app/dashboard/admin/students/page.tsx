...
export default function ManageStudents() {
    ...
    const loadClassified = async () => {
      try {
        ...
        const res = await api.get(
          `/admin/students/classified?${params.toString()}`
        );
  ...
  const mediums = useMemo(
    () => Array.from(new Set(rows.map((r) => r.medium))).sort(),
    [rows]
  );
      ...
      <Tabs defaultValue="selected">
        ...
        <TabsContent value="selected" className="space-y-4 mt-4">
          ...
          <p className="text-gray-600">
            View selected students by subject and medium, with mapped tutors
          </p>
          ...
          <div className="overflow-x-auto rounded-md border bg-white">
            <table className="min-w-full divide-y">
              ...
              <tbody className="divide-y">
                {filtered.map((r, i) => (
                  <tr
                    key={`${r.applicationId}-${r.subject}-${r.medium}-${i}`}
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-sm">
                      <Badge variant="outline">{r.medium}</Badge>
                    </td>
                    <td className="px-4 py-2 text-sm">
                    ...
                    <td className="px-4 py-2 text-sm">
                      <Badge
                        variant={
                          (r as any).dropoutRisk === "Perfect"
                            ? "default"
                            : (r as any).dropoutRisk === "Maybe Dropout"
                            ? "outline"
                            : (r as any).dropoutRisk === "Sure Dropout"
                            ? "destructive"
                            : "secondary"
                        }
                        className={
        ...
        <TabsContent value="classified" className="space-y-4 mt-4">
          ...
          <p className="text-gray-600">
            Classified by grade with full details from initial application.
          ...
          <div className="flex flex-col md:flex-row gap-3 md:items-center">
            ...
            <select
              ...
              className="w-full md:w-40 rounded-md border px-3 py-2"
              value={classSubj}
              onChange={(e) => setClassSubj(e.target.value)}
            >
              <option value="all">All Subjects</option>
              {Array.from(new Set(classified.flatMap((c) => c.subjects))).map(
                (s) => (
                  <option key={s} value={s}>
                    {s}
                  </option>
                ...
                {classified.map((c) => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-sm">
                      <Badge
                        variant={
                          c.dropoutRisk === "Perfect"
                            ? "default"
                            : c.dropoutRisk === "Maybe Dropout"
                            ? "outline"
                            : c.dropoutRisk === "Sure Dropout"
                            ? "destructive"
                            : "secondary"
                        }
                        className={
