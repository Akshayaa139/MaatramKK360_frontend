...
export default function AdminApplicationDetailsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const res = await api.get(`/admin/applications/${idParam}`);
        setData(res.data);
        const vols = await api.get(`/admin/users?role=volunteer&limit=100`);
        const alums = await api.get(`/admin/users?role=alumni&limit=100`);
        const vrows = (
          Array.isArray(vols.data?.users)
            ? vols.data.users
        ...
        const arows = (
          Array.isArray(alums.data?.users)
            ? alums.data.users
            : Array.isArray(alums.data)
            ? alums.data
            : []
        ) as any[];
        setVolunteers(
        ...
        const t = await api.get("/admin/tutors/details");
        const trows = Array.isArray(t.data) ? t.data : [];
        setTutors(
          trows.map((r: any) => ({ _id: r.id, name: r.name, email: r.email }))
        );
      } catch (e: any) {
  ...
  const assignTutor = async () => {
    ...
    await api.post(`/admin/applications/${data._id}/assign-tutor`, {
      tutorId: tutorAssignId,
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="space-y-1 text-sm">
            <div>
              Father: {f.fatherName}{" "}
              {f.fatherOccupation ? `• ${f.fatherOccupation}` : ""}
            </div>
            <div>
              Mother: {f.motherName}{" "}
              {f.motherOccupation ? `• ${f.motherOccupation}` : ""}
            </div>
        ...
        <CardContent className="space-y-3 text-sm">
          {data?.televerification ? (
            <div className="space-y-1">
              <div>
                Status:{" "}
                <Badge variant="outline">{data.televerification.status}</Badge>
              </div>
              {data.televerification.recommendation && (
                <div>
                  Recommendation: {data.televerification.recommendation}
                </div>
              )}
              {data.televerification.remarks && (
                <div>Remarks: {data.televerification.remarks}</div>
              )}
              {data.televerification.volunteer && (
                <div>
                  Volunteer: {data.televerification.volunteer.name} (
                  {data.televerification.volunteer.email})
                </div>
              )}
            </div>
          ) : (
            <div className="text-muted-foreground">
              No tele-verification assigned
            </div>
          )}
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="text-muted-foreground">
            Assign a tutor and schedule based on tutor availability
          </div>
          <div className="grid grid-cols-3 gap-3 items-end">
            <div className="col-span-2">
              ...
              <select
                ...
                className="w-full border rounded-md px-3 py-2"
                value={tutorAssignId}
                onChange={(e) => setTutorAssignId(e.target.value)}
              >
          ...
          {data?.tutorAssignment && (
            <div className="mt-3">
              <div>
                Meeting:{" "}
                {data.tutorAssignment.meetingLink ? (
                  <a
                    href={data.tutorAssignment.meetingLink}
                    className="text-blue-600 underline"
                    target="_blank"
                    rel="noreferrer"
                  >
              ...
              <div>
                Schedule: {data.tutorAssignment.schedule?.day}{" "}
                {data.tutorAssignment.schedule?.startTime} -{" "}
                {data.tutorAssignment.schedule?.endTime}
              </div>
            </div>
          )}
        </CardContent>
