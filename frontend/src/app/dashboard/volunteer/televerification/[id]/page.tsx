...
export default function TeleverificationDetailsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // api.ts handles token automatically
        const res = await api.get(`/televerification/${id}/details`);
  ...
  const submit = async () => {
    try {
      await api.put(`/televerification/${id}`, {
        ...form,
        status: "Completed",
      });
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            <div>
              Class: {application?.educationalInfo?.currentClass || "—"}
            </div>
            ...
            <div>
              Subjects:{" "}
              {(application?.educationalInfo?.subjects || [])
                .map((s: any) => s?.name || s)
                .join(", ") || "—"}
            </div>
            <div>
              10th %: {application?.educationalInfo?.tenthPercentage || "—"}
            </div>
            <div>
              Current %:{" "}
              {application?.educationalInfo?.currentPercentage || "—"}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            <div>
              Father: {application?.familyInfo?.fatherName || "—"} (
              {application?.familyInfo?.fatherOccupation || "—"})
            </div>
            <div>
              Mother: {application?.familyInfo?.motherName || "—"} (
        ...
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            ...
            <div>
              Challenges: {application?.additionalInfo?.challenges || "—"}
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, callDurationMinutes: e.target.value })
                  }
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                {["selected", "rejected", "waitlist", "neutral"].map((r) => (
                  <Button
                    key={r}
                    variant={form.recommendation === r ? "default" : "outline"}
                    size="sm"
                    onClick={() => setForm({ ...form, recommendation: r })}
                  >
                    {r}
                  </Button>
                ))}
