...
const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  const pendingTeleverifications = await Televerification.countDocuments({ status: 'pending' });
  const completedTeleverifications = await Televerification.countDocuments({ status: 'completed' });
...
const getRecentActivities = asyncHandler(async (req, res) => {
  ...
  const activities = [
    ...
    ...recentEvaluations.map(eval => ({
      ...
      description: `${eval.application.studentName} - ${eval.recommendation}`,
...
const getPanelStats = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    evaluationStats,
    batchStats: batchStats[0] || { totalBatches: 0, avgStudentsPerBatch: 0, totalStudents: 0 }
  });
});
